...
// E7 · Clarification Layer — render câu hỏi hỏi lại người dùng trước khi generate (docs/
...
// đơn giản: có `options` → single_choice (nút bấm chọn 1), không có → input tự do (free_text).
...
//
// KHÔNG có countdown/tự disable khi "hết hạn" (redesign lần 2, 2026-07-14 — xem explainer.md
// § 6.7): session E7 giờ được lưu bền (Supabase/file), user có thể trả lời bất cứ lúc nào, không
// bị giới hạn bởi 1 kết nối HTTP còn sống. Nút gửi LUÔN có thể bấm (miễn field required đã điền) —
// nếu server từ chối thật lúc submit (đã hết hạn/đã trả lời trước đó), lỗi hiển thị qua
// ChatItem.status === "error" ở tầng cha (ChatMessageItem.tsx), không phải component này tự đoán
// trước khi bấm.
