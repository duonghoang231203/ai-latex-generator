// lib/prompt-eval/providers/engineering-real-ai-provider.ts
// Gọi AI THẬT qua getProvider() factory (đọc .env) + generateWithTruncationRecovery() — song song
// physics-real-ai-provider.ts nhưng template="engineering". TỐN API QUOTA THẬT mỗi lần chạy. Đo
// compliance thật của prompt `engineering` (article + siunitx cho đơn vị, circuitikz cho mạch…).
...
export default class EngineeringRealAiProvider implements ApiProvider {
  id(): string {
    return "engineering-real-ai-provider";
  }
