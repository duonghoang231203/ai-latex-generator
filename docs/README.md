...
| [backend-roadmap.md](./backend-roadmap.md) | Lộ trình riêng cho backend/orchestrator (Next.js API + `lib/`). |
| [frontend-roadmap.md](./frontend-roadmap.md) | Lộ trình riêng cho frontend (`components/`, UX). |
| [feature-tracking.md](./feature-tracking.md) | Bảng theo dõi tiến độ chi tiết từng tính năng đang triển khai. |
| [features/](./features/) | **Nghiên cứu tính năng** (mỗi feature 1 folder): tìm hiểu vấn đề → research giải pháp → cách tiếp cận. Hiện có E1–E7 (multi-file project, agentic assembly, RAG, formula OCR, Markdown→LaTeX, prompt engineering, clarification layer). |
