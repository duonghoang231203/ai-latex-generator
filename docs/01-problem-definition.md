...
- Preview PDF + tải về. **Lưu trữ tài liệu (file-based) + CRUD + chỉnh sửa (mã nguồn/chat)**; chưa có auth.
...
- Tài khoản/auth, **DB chia sẻ & collaboration**, self-host/local-first (lưu trữ file-based cơ bản đã có).
